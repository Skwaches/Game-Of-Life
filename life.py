...
class Grid():
    def __init__(self, position, divisions, size, color, gridColor, outline):
        ...
        self.cell = pygame.math.Vector2(
                self.size.x/self.divisions.x,
                self.size.y/self.divisions.y
                )
        ...
        self.state = [
                [False for i in range(int(self.divisions.y))]
    ...
    def rngState(self):
        self.state = [
                [
                 True if random.uniform(0.0, 1.0) > random.uniform(0.3, 1.0)
                 else False
    ...
    def set(self, position, newState, radius=0):
        x_ = int((position[0] - self.position.x)/self.cell.x)
        y_ = int((position[1] - self.position.y)/self.cell.y)
        x, y = x_, y_
    ...
    def update(self):
        ...
        for x in range(int(self.divisions.x)):
            for y in range(int(self.divisions.y)):
                ...
                for i, j in indices:
                    if i >= 0 and j >= 0:
                        if i < self.divisions.x and j < self.divisions.y:
...
def init():
    global size, grid, updateSpeed, updateDelay,  \
          running, left_Held, right_Held, update, \
          animate, UPDATE_EVENT
